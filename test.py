...
# Initialize Chips Agent for quick replies
chips_agent = Agent(
...
def build_context_string(messages: list[dict]) -> str:
    """Convert chat history into a context string"""
    if not messages:
        return ""
...
async def chat(request: ChatRequest):
    """Chat endpoint that uses the gift recommendation agent"""
    try:
        ...
        # Get chips from chips_agent with error handling
        chips_list = []
        try:
            chips_reply = await chips_agent.run(full_input)
            chips_text = chips_reply.output.strip()
            
            # Parse chips (split by "|" and strip whitespace)
            if chips_text:
                chips_list = [chip.strip() for chip in chips_text.split("|") if chip.strip()]
                # Ensure we have exactly 3 chips
                chips_list = chips_list[:3]  # Take first 3 if more
